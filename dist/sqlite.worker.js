...
const mod = {
    ...
    async SplitFileHttpDatabase(wasmUrl, configs, mainVirtualFilename, maxBytesToRead = Infinity) {
        ...
        for (const { config, virtualFilename } of hydratedConfigs) {
            ...
            console.log("sql log");
            console.log(sql);
            const lazyFile = sql.FS_createLazyFile(sql, "/", filename, true, true, {
                rangeMapper,
...
stream:{}},Hb:{node:{Ta:P.Ga.Ta,Pa:P.Ga.Pa},stream:x.dc}});c=x.createNode(a,b,c,d);U(c.mode)?(c.Ga=P.Wa.dir.node,c.Ha=P.Wa.dir.stream,c.Ia={}):x.isFile(c.mode)?(c.Ga=P.Wa.file.node,c.Ha=P.Wa.file.stream,c.Ma=0,c.Ia=null):40960===(c.mode&61440)?(c.Ga=P.Wa.link.node,c.Ha=P.Wa.link.stream):8192===(c.mode&61440)&&(c.Ga=P.Wa.Hb.node,c.Ha=P.Wa.Hb.stream);c.timestamp=Date.now();a&&(a.Ia[b]=c,a.timestamp=c.timestamp);return c},xc(a){return a.Ia?a.Ia.subarray?a.Ia.subarray(0,a.Ma):new Uint8Array(a.Ia):new Uint8Array(0)},
...
b,c,d,f){if(!x.isFile(a.node.mode))throw new x.Fa(43);a=a.node.Ia;if(f&2||a.buffer!==r.buffer){if(0<c||c+b<a.length)a.subarray?a=a.subarray(c,c+b):a=Array.prototype.slice.call(a,c,c+b);c=!0;b=zb(b);if(!b)throw new x.Fa(48);r.set(a,b)}else c=!1,b=a.byteOffset;return{Sb:b,Db:c}},cb(a,b,c,d){P.Ha.write(a,b,0,d,c,!1);return 0}}},Bb=(a,b,c)=>{var d=`al ${a}`;xa(a).then(f=>{b(new Uint8Array(f));d&&Va(d)},()=>{if(c)c();else throw`Loading data file "${a}" failed.`;});d&&Sa(d)},Cb=e.preloadPlugins||[],Db=
(a,b,c,d)=>{"undefined"!=typeof Browser&&Browser.ib();var f=!1;Cb.forEach(k=>{!f&&k.canHandle(b)&&(k.handle(a,b,c,d),f=!0)});return f},Eb=(a,b,c,d,f,k,l,m,u,p)=>{function A(K){function L(J){p?.();m||x.kb(a,b,J,d,f,u);k?.();Va(C)}Db(K,q,L,()=>{l?.();Va(C)})||L(K)}var q=b?lb(N(a+"/"+b)):a,C=`cp ${q}`;Sa(C);"string"==typeof c?Bb(c,A,l):A(c)},Fb=(a,b)=>{var c=0;a&&(c|=365);b&&(c|=146);return c};function ub(a,b){x.Lb[a]={Ha:b}}function U(a){return 16384===(a&61440)}
...
function Ib(a){for(var b;;){if(x.Qb(a))return a=a.Na.Rb,b?"/"!==a[a.length-1]?`${a}/${b}`:a+b:a;b=b?`${a.name}/${b}`:a.name;a=a.parent}}function Hb(a,b){for(var c=0,d=0;d<b.length;d++)c=(c<<5)-c+b.charCodeAt(d)|0;return(a+c>>>0)%x.Va.length}function Jb(a){var b=Hb(a.parent.id,a.name);a.ab=x.Va[b];x.Va[b]=a}function Kb(a){var b=Hb(a.parent.id,a.name);if(x.Va[b]===a)x.Va[b]=a.ab;else for(b=x.Va[b];b;){if(b.ab===a){b.ab=a.ab;break}b=b.ab}}
function Lb(a){var b=["r","w","rw"][a&3];a&512&&(b+="w");return b}function Gb(a,b){if(x.Ob)return 0;if(!b.includes("r")||a.mode&292){if(b.includes("w")&&!(a.mode&146)||b.includes("x")&&!(a.mode&73))return 2}else return 2;return 0}function Mb(a,b){try{return V(a,b),20}catch(c){}return Gb(a,"wx")}function Nb(a,b,c){try{var d=V(a,b)}catch(f){return f.Ka}if(a=Gb(a,"wx"))return a;if(c){if(!U(d.mode))return 54;if(x.Qb(d)||Ib(d)===x.cwd())return 10}else if(U(d.mode))return 31;return 0}
...
var x={root:null,jb:[],Lb:{},streams:[],oc:1,Va:null,Kb:"/",ub:!1,Ob:!0,Fa:class{constructor(a){this.name="ErrnoError";this.Ka=a}},sb:{},hc:null,mb:0,Vb:class{constructor(){this.Xa={};this.node=null}get object(){return this.node}set object(a){this.node=a}get flags(){return this.Xa.flags}set flags(a){this.Xa.flags=a}get position(){return this.Xa.position}set position(a){this.Xa.position=a}},Ub:class{constructor(a,b,c,d){a||=this;this.parent=a;this.Na=a.Na;this.Ra=null;this.id=x.oc++;this.name=b;this.mode=
c;this.Ga={};this.Ha={};this.rdev=d}get read(){return 365===(this.mode&365)}set read(a){a?this.mode|=365:this.mode&=-366}get write(){return 146===(this.mode&146)}set write(a){a?this.mode|=146:this.mode&=-147}get nc(){return U(this.mode)}get mc(){return 8192===(this.mode&61440)}},createNode(a,b,c,d){a=new x.Ub(a,b,c,d);Jb(a);return a},Qb(a){return a===a.parent},isFile(a){return 32768===(a&61440)},isFIFO(a){return 4096===(a&61440)},isSocket(a){return 49152===(a&49152)},Xb:4096,kc:a=>x.streams[a],dc:{open(a){a.Ha=
x.jc(a.node.rdev).Ha;a.Ha.open?.(a)},Ua(){throw new x.Fa(70);}},vb:a=>a>>8,Bc:a=>a&255,Za:(a,b)=>a<<8|b,jc:a=>x.Lb[a],Tb(a,b){function c(l){x.mb--;return b(l)}function d(l){if(l){if(!d.fc)return d.fc=!0,c(l)}else++k>=f.length&&c(null)}"function"==typeof a&&(b=a,a=!1);x.mb++;1<x.mb&&Ca(`warning: ${x.mb} FS.syncfs operations in flight at once, probably just doing extra work`);var f=Qb(x.root.Na),k=0;f.forEach(l=>{if(!l.type.Tb)return d(null);l.type.Tb(l,a,d)})},Na(a,b,c){var d="/"===c;if(d&&x.root)throw new x.Fa(10);
if(!d&&c){var f=W(c,{rb:!1});c=f.path;f=f.node;if(f.Ra)throw new x.Fa(10);if(!U(f.mode))throw new x.Fa(54);}b={type:a,Dc:b,Rb:c,jb:[]};a=a.Na(b);a.Na=b;b.root=a;d?x.root=a:f&&(f.Ra=b,f.Na&&f.Na.jb.push(b));return a},Fc(a){a=W(a,{rb:!1});if(!a.node.Ra)throw new x.Fa(28);a=a.node;var b=a.Ra,c=Qb(b);Object.keys(x.Va).forEach(d=>{for(d=x.Va[d];d;){var f=d.ab;c.includes(d.Na)&&Kb(d);d=f}});a.Ra=null;a.Na.jb.splice(a.Na.jb.indexOf(b),1)},lookup(a,b){return a.Ga.lookup(a,b)},Ya(a,b,c){var d=W(a,{parent:!0}).node;
...
c+". Status: "+O.status);ha=void 0!==O.response?new Uint8Array(O.response||[]):na(O.responseText||"",!0);Ua[X]=ha}if("undefined"==typeof M.Xa[X])throw Error("doXHR failed!");return M.Xa[X]});if(q||!C)J=C=1,J=C=this.nb(0).length,Ba("LazyFiles on gzip forces download of the whole file when length is accessed");this.Bb=C;this.Ab=J;this.ob=!0}get length(){this.ob||this.Cb();return this.Bb}get chunkSize(){this.ob||this.Cb();return this.Ab}}if("undefined"!=typeof XMLHttpRequest){if(!ba)throw"Cannot do synchronous binary XHRs outside webworkers in modern browsers. Use --embed-file or --preload-file in emcc";
var m=new l;var u=void 0}else u=c,m=void 0;var p=Sb(a,b,d,f);m?p.Ia=m:u&&(p.Ia=null,p.url=u);Object.defineProperties(p,{Ma:{get:function(){return this.Ia.length}}});var A={};Object.keys(p.Ha).forEach(q=>{var C=p.Ha[q];A[q]=(...K)=>{Tb(p);return C(...K)}});A.read=(q,C,K,L,J)=>{Tb(p);return k(q,C,K,L,J)};A.$a=(q,C,K)=>{Tb(p);var L=zb(C);if(!L)throw new x.Fa(48);k(q,r,L,C,K);return{Sb:L,Db:!0}};p.Ha=A;return p}};
...
0,7,5,1,1,102,0,0);b=new WebAssembly.Module(new Uint8Array(b));b=(new WebAssembly.Instance(b,{e:{f:a}})).exports.f}Bc.set(c,b)}zc.set(a,c);return c},Cc=x.Jb,Dc=x.Ib,Ec=x.Sa;x.ec=Eb;[44].forEach(a=>{x.sb[a]=new x.Fa(a);x.sb[a].stack="<generic error, no stack>"});x.Va=Array(4096);x.Na(P,{},"/");x.mkdir("/tmp");x.mkdir("/home");x.mkdir("/home/web_user");
...
(function(){x.mkdir("/proc");var a=x.mkdir("/proc/self");x.mkdir("/proc/self/fd");x.Na({Na(){var b=x.createNode(a,"fd",16895,73);b.Ga={lookup(c,d){var f=Y(+d);c={parent:null,Na:{Rb:"fake"},Ga:{readlink:()=>f.path}};return c.parent=c}};return b}},{},"/proc/self/fd")})();x.hc={MEMFS:P};e.FS_createPath=x.Jb;e.FS_createDataFile=x.kb;e.FS_createPreloadedFile=x.ec;e.FS_unlink=x.unlink;e.FS_createLazyFile=x.Ib;e.FS_createDevice=x.Sa;
